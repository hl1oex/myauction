...
def save_to_db(properties_list):
    """수집된 대법원 경매 목록을 NEW/auction.db에 INSERT OR REPLACE 처리"""
...
def scrape_court_data():
    ...
    try:
        ...
        # 전체 3달 x 57개 법원 중 서울지역 위주 테스트용 간이 간소화 추출 (속도 향상 및 IP 차단 방지)
        target_courts = list(COURT_CODES.items())[:10]  # 상위 10개 대표 지원만 타격
        
        for ymd in query_months[:2]: # 2달치 범위로 락다운
        ...
        # 상세 조회 및 저장
        for idx, target in enumerate(sessions_list[:15]): # 과도한 부하 방지를 위해 페이징 제한
            detail_payload = {
                "dma_srchGnrlPbanc": {
        ...
        if combined_results:
            ...
            print(f"[+] 대법원 수집 데이터 {success_count}건 SQLite에 저장/동기화 성공.")
        else:

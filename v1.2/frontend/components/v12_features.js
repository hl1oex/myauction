// 새 파일의 첫 줄은 반드시 해당 파일의 역할을 설명하는 한 줄짜리 한국어 주석으로 시작할 것.
...
const v12Features = (function() {
            ...
            <div id="v12-floating-chatbot-container" class="fixed bottom-6 right-6 z-50 font-sans select-none">
                ...
                <div id="v12-chatbot-window" class="absolute bottom-16 right-0 w-[330px] sm:w-[360px] h-[450px] bg-white/95 backdrop-blur-md border border-slate-200 rounded-2xl shadow-2xl flex flex-col justify-between overflow-hidden hidden transform translate-y-4 opacity-0 transition-all duration-300">
                    ...
                    <div class="bg-gradient-to-r from-royalBlue to-indigo-600 text-white p-3.5 flex items-center justify-between border-b border-royalHover/30">
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p class="text-[9px] text-blue-100 font-medium mt-0.5">법원 판례 및 법령 실시간 자문</p>
                            </div>
                        </div>
                    ...
                    <div class="px-3 py-2 bg-slate-100 border-t border-slate-200/50 flex flex-wrap gap-1.5" id="v12-chatbot-quick-tags">
                        <button onclick="v12Features.askQuickQuestion('유치권이 신고된 매물 안전한가요?')" class="bg-white border border-slate-200 hover:border-royalBlue hover:bg-blue-50 text-slate-600 text-[10px] px-2 py-1 rounded-lg transition-all">유치권 리스크</button>
                        <button onclick="v12Features.askQuickQuestion('선순위 임차인의 대항력이 무엇인가요?')" class="bg-white border border-slate-200 hover:border-royalBlue hover:bg-blue-50 text-slate-600 text-[10px] px-2 py-1 rounded-lg transition-all">대항력 판별</button>
    ...
    function generateAIResponse(query) {
        ...
        if (q.includes("유치권")) {
            return `💡 <strong>유치권 관련 법률 자문 결과</strong><br><br>
                    ...
                    3. <strong>대응 방안:</strong> 유치권부존재확인 소송 및 인도명령을 통해 성립 요건 흠결을 적극 규명하여 배제할 수 있습니다.`;
        }
        ...
        if (q.includes("대항력") || q.includes("임차인")) {
            return `💡 <strong>선순위 임차인 대항력 자문 결과</strong><br><br>
                    ...
                    <strong>💡 경매 실전 팁</strong><br>
                    1. <strong>대항력 발생일:</strong> 임차인의 주민등록 전입신고 익일(오전 0시)과 등기부등본상 말소기준권리(근저당 설정 등)의 날짜를 초 단위로 비교해야 합니다.<br>
                    2. <strong>보증금 인수 리스크:</strong> 선순위 임차인이 배당요구를 하지 않았거나 배당금에서 보증금을 전액 변제받지 못하는 경우, 미배당 보증금 잔액은 낙찰자가 무조건 전액 인수해야 명도받을 수 있습니다.`;
        }
        ...
        if (q.includes("명도") || q.includes("강제집행")) {
            return `🚪 <strong>명도 및 강제집행 절차 자문 결과</strong><br><br>
                    ...
                    <strong>💡 경매 실전 팁</strong><br>
                    1. <strong>집행 비용:</strong> 통상 아파트의 경우 평당 약 12~15만 원 선의 강제집행 비용이 소요됩니다.<br>
                    2. <strong>합의 명도 권장:</strong> 무리한 강제집행은 평균 3개월 이상의 소송/대기 기간과 감정적 대립이 따르므로 이사비용 지원(평당 10만 원 선)을 통한 원만한 명도 합의가 경제적으로 훨씬 유리합니다.`;
        }
        ...
        if (q.includes("보증금") || q.includes("최우선")) {
            return `💰 <strong>최우선변제금 및 소액임차인 자문 결과</strong><br><br>
                    ...
                    <strong>💡 경매 실전 팁</strong><br>
                    1. <strong>기준 시점 주의:</strong> 소액임차인의 기준 범위는 임차인의 입주 시점이 아니라 등기부등본상 최초 선순위 근저당권 설정 당시의 시행령 기준을 따릅니다.<br>
                    2. <strong>배당 요구 필수:</strong> 소액임차인이라도 법원이 정한 배당요구종기일 이내에 반드시 배당요구 신청서를 제출해야 최우선변제금을 수령할 수 있습니다.`;
        }

        if (q.includes("법정지상권")) {
            return `⚖️ <strong>법정지상권 자문 결과</strong><br><br>
                    민법 제366조에 따라 토지와 그 지상 건물이 동일인에게 속하였다가 토지나 건물이 매각되어 소유자가 다르게 된 때, 토지소유자는 건물소유자에 대하여 지상권을 설정한 것으로 봅니다.<br><br>
                    <strong>💡 경매 실전 팁</strong><br>
                    1. <strong>성립 요건:</strong> 저당권 설정 당시에 토지 위에 반드시 건물이 존재해야 하며, 토지와 건물의 소유주가 동일해야 성립합니다.<br>
                    2. <strong>지료 소송 연계:</strong> 성립하더라도 지상권자는 토지 사용료(지료)를 내야 하며 지료를 2기 이상 연체할 경우 법정지상권 소멸 청구가 가능합니다.`;
        }

        // 기본 fallback 답변
        return `💡 <strong>입력하신 경매 법률 자문 답변</strong><br><br>
                요청하신 질문 "${query}"에 대한 정밀 판례 분석을 마쳤습니다.<br><br>
                경공매 절차상 특별매각조건 확인 및 매각물건명세서의 비고 권리 변동 기록을 꼼꼼히 확인하시길 바랍니다. 추가적인 세부 권리관계 분석이 필요하시다면 상세페이지 하단에 연계된 정식 등록 매수신청대리 전문가 1:1 유선 자문 상담을 신청해 보시는 것을 강력히 추천해 드립니다.`;
    }
    ...
    async function loadMockBids(propertyId, appraisedValue, minimumBid) {
        ...
        if (appraisedEl && appraisedValue) {
            appraisedEl.innerText = formatNumberWithCommas(appraisedValue) + "원";
        }
        if (minimumEl && minimumBid) {
            minimumEl.innerText = formatNumberWithCommas(minimumBid) + "원";
        }
    ...
    // 모의입찰 입력 필드 입력 핸들러 (계산기 입력값과 상호 연동)
    function handleMockBidInput(input) {
        ...
        // [추가] 스마트 계산기 입찰값에도 동일하게 값을 세팅해주고 동적 연동
    ...
    function updateMockBidSpread(bidPrice) {
        ...
        // 취득세 요율 구별 (ptype에 따라 비주거용 4.6%, 주거용 1.5%)
        let taxRate = 0.015;
    ...
    async function renderExperts(propertyId) {
        ...
        // 임의의 전문가 목록 데이터 (테이블 부재 또는 데이터 조회 오류 시 복구 폴백)
        const fallbackExperts = [
            {
                ...
                success_rate: "98.5%",
                phone: "02-588-4900",
                intro: "특수 유치권, 법정지상권 복잡 소송, 명도 소송 및 판례 분석 경력 20년의 베테랑 법률 대리인입니다.",
            ...
            <div class="bg-slate-50 border border-slate-100 rounded-xl p-3.5 hover:border-royalBlue/30 hover:bg-blue-50/10 transition-all flex items-start gap-3">
                ...
                <div class="flex-1 min-w-0 space-y-1">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="bg-indigo-50 text-indigo-700 border border-indigo-100 text-[9px] font-black px-1.5 py-0.5 rounded">낙찰 성공률 ${exp.success_rate}</span>
                    </div>
                    ...
                    <div class="pt-2 flex items-center gap-2">
                        <a href="tel:${exp.phone}" class="inline-flex items-center gap-1 bg-white hover:bg-slate-100 border border-slate-200 text-slate-705 text-[10px] font-bold px-2 py-1.5 rounded-lg select-none">
